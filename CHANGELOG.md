...
- `KrynoxCaptchaField` and `KrynoxCaptchaWidget` for Django forms.
- Pass the request with `SignupForm(request.POST, request=request)` to forward the
  visitor IP from `REMOTE_ADDR`.

[0.1.0]: https://github.com/krynox-security/plugin-django/releases/tag/v0.1.0
